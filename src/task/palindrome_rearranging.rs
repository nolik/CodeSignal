...
//Solution used LinkedHashMap from cargo: linked-hash-map = "0.5.1"
pub fn palindrome_rearranging(input_string: String) -> bool {
    let mut letters = LinkedHashMap::new();
    let mut unique_index_of_char = 0;
    for ch in input_string.chars() {
        let counter = letters.entry(ch).or_insert(0);
        *counter += 1;
    }
    for char_item in letters.values() {
        if char_item % 2 != 0 {
            if unique_index_of_char == 1 {
                return false;
            } else {
                unique_index_of_char += 1;
            }
        }
    }
    true
