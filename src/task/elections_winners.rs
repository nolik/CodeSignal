...
*/
pub fn electionsWinners(votes: Vec<i32>, k: i32) -> i32 {
    let current_winner = votes.iter().max().unwrap();

    if k == 0 {
        return (votes.iter().filter(|&x| x == current_winner).count() == 1) as i32;
    } else {
        return votes.iter().filter(|&&item| item + k > *current_winner).count() as i32;
    }
}
